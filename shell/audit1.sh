#!/bin/sh 
 ...
 if   [ "$os" = "SunOS" ] ; then  
   ...
   # added to check for account logout
 ...
 if [ "$os" = "SunOS" ] ; then 
	echo "List all service instances" 
	run svcs -a
 fi;
 ...
 # PTL temp remove for e-topup # xauth list 
 ...
 if   [ "$os" = "SunOS" ] ; then  
   ...
   $echo "cron.allow:" 
   cat  /var/spool/cron/cron.allow 2>/dev/null 
   $echo "cron.allow:" 
   cat /etc/at.allow   2>/dev/null 
   $echo "cron.deny:" 
   cat /var/spool/cron/cron.deny   2>/dev/null 
   $echo "at.deny:" 
   cat   /etc/at.deny   2>/dev/null 
  
 elif   [ "$os" = "HP-UX" ] ; then  
   tail -500 /var/adm/cron/log 
   $echo "cron.allow:" 
 ...
 for log in $logs ; do 
   if [ -s /var/adm/$log ] ; then 
     $echo "\nTail of /var/adm/$log..." 
     tail -500 /var/adm/$log |egrep -v "MARK" 2>/dev/null 
   fi 
 done 
 ...
 for log in $logs ; do 
   if [ -s /var/log/$log ] ; then 
     $echo "\nTail of /var/log/$log..." 
     tail -500 /var/log/$log |egrep -v "MARK" 2>/dev/null 
   fi 
 done 
 ...
 if   [ "$os" = "Linux" ] ; then  
   run faillog -a |head -500  2>/dev/null 
 fi 
 ...
 for f in $files; do 
   if [ -f $f ] ; then  
     $echo "Mysql logs tail -500 $f .." 
     tail -500 $f | egrep  -v "$comments" 2>/dev/null 
   fi 
 done; 
